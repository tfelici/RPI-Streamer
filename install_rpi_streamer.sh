...
# flask_app.service runs as root and drops down to the owning user via "sudo -u $OWNER"
# to check/apply updates (see /system-check-update and /system-do-update routes). That
# script then itself runs "sudo git fetch/reset/clean" etc as $OWNER. Without NOPASSWD
# for both root (the outer switch) and $OWNER (the inner calls), sudo blocks waiting for
# a password with no TTY available, which fails silently and breaks update checks with
# "No valid JSON found in install script output".
printf "Granting passwordless sudo to root and %s (needed for update checks)...\n" "$USER"
...
echo "$USER ALL=(ALL) NOPASSWD: ALL" | sudo tee /etc/sudoers.d/010_owner-nopasswd >/dev/null
sudo chmod 440 /etc/sudoers.d/010_owner-nopasswd
