...
def get_gnss_location():
    """
    Get GNSS location using gpsd daemon via python-gps library.    
    Uses persistent connection for better performance when called frequently.
    ...
    return _get_gps_manager().get_location()


# Global GPS connection manager for persistent connections
_gps_manager = None

class GPSConnectionManager:
    """
    Manages persistent connection to gpsd daemon for better performance.
    ...
    def __init__(self):
        ...
        self.error_backoff = 5.0  # Wait 5 seconds after connection error
        
    def _connect(self):
        """Establish connection to gpsd if not already connected"""
        try:
            if self.session is None:
                import gps
                self.session = gps.gps(mode=gps.WATCH_ENABLE)
                # Small delay to let gpsd initialize the connection
                time.sleep(0.1)
                return True
            return True
    ...
    def get_location(self):
        """
        Get current GPS location using persistent connection.
        Handles connection errors and reconnection automatically.
        """
        # Check if we're in error backoff period
        current_time = time.time()
        if (self.last_error_time > 0 and 
            (current_time - self.last_error_time) < self.error_backoff):
            return False, {'error': 'GPS connection in error backoff period'}
        
        try:
            # Ensure we have a connection
            if not self._connect():
                return False, {'error': 'Failed to connect to gpsd'}
            
            # Quick timeout for frequent polling
            timeout = 1.0
            ...
            while (time.time() - start_time) < timeout:
                try:
                    ...
                    report = self.session.next(timeout=0.3)
                    
                    # Look for TPV (Time-Position-Velocity) report
                    if report['class'] == 'TPV':
                        # Check if we have a valid fix
                        if hasattr(report, 'mode') and report.mode >= 2:
                            # We have at least 2D fix
                            location_data = {
                                'fix_status': 'valid',
                                ...
                                'course': getattr(report, 'track', None),
                                'fix_type': '3D' if report.mode == 3 else '2D',
                                'timestamp': getattr(report, 'time', time.time())
                            }
                            
                            # Get satellite info if available (quick attempt)
                            satellites = {'used': 0, 'total': 0}
                            try:
                                # Quick attempt to get SKY report
                                sky_report = self.session.next(timeout=0.1)
                                if sky_report['class'] == 'SKY':
                                    satellites['total'] = len(getattr(sky_report, 'satellites', []))
                                    satellites['used'] = getattr(sky_report, 'uSat', 0)
                                    location_data['hdop'] = getattr(sky_report, 'hdop', None)
                            except:
                                # No satellite data available - don't wait
                                pass
                            
                            location_data['satellites'] = satellites
                            
                            # Connection successful - reset error timer
                            self.last_error_time = 0
                    ...
                    # Handle individual report errors
            ...
            # Timeout reached - but connection is still good
            return False, {'error': 'Timeout waiting for GPS data'}
            
        except ImportError:
            return False, {'error': 'python3-gps library not available'}
        except Exception as e:
            # Connection error - close connection and set backoff
            self._disconnect()
            self.last_error_time = time.time()
            return False, {'error': f'GPS error: {str(e)}'}
    
    def close(self):
        """Clean shutdown of GPS connection"""
        self._disconnect()


def _get_gps_manager():
    """Get or create the global GPS connection manager"""
    global _gps_manager
    if _gps_manager is None:
        _gps_manager = GPSConnectionManager()
    return _gps_manager


# Clean up GPS connection on exit
import atexit
def _cleanup_gps():
    global _gps_manager
    if _gps_manager:
